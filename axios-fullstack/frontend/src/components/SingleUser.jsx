...
const SingleUser = ({ username }) => {
  ...
  if (loading)
    return <p className="text-orangeDark text-center mt-8">Loading...</p>;
  if (error) return <p className="text-red-500 text-center mt-8">{error}</p>;

  return (
    <div className="bg-zinc-100 border-2 p-4 rounded-lg flex flex-col items-center text-orangeDark">
      <img
        src={user.avatar_url}
        alt={user.login}
